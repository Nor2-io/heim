...
Heim currently supports creating applications with `C#`, `Python`, `JavaScript` / `TypeScript`, `Rust`, `C` and `Go`.<br />
...
We compile the applications to a <Icon name="seti:wasm" label="C# language icon" class="inline-icon" /> [WebAssembly Component Model](https://component-model.bytecodealliance.org/), with Heim and deploys this wasm-file directly to the Heim-runtime either locally or to our cloud-instances.
...
Applications also need to be set up with a specific application configuration file, the [application.toml](./../../application/config/applicationtoml/) file, which defines the [application](#applicationtoml).
...
You can read more about this file in the [application.toml](./../../application/config/applicationtoml/) document.
...
You can read more about this configuration file in the [component.toml](./../../application/config/componenttoml/) document.
